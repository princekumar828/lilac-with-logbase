...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='LILAC Sampling Algorithm')
    parser.add_argument('--dataset', type=str, default=None,
                        help='Specific dataset to process (e.g., Proxifier). If not specified, all datasets will be processed.')
    args = parser.parse_args()
    
    # Handle specific dataset selection
    if args.dataset:
        if args.dataset not in datasets:
            print(f"Error: Dataset '{args.dataset}' not found in available datasets.")
    ...
    for dataset in datasets_to_process:
        ...
        # Simple improvement: stratified sampling to ensure template diversity
        k_rate = 0.2
        target_size = int(k_rate * len(labelled_logs))
        
        # Get at least one example of each template, then fill remaining with random
        unique_templates = labelled_logs['EventTemplate'].unique()
        
        if len(unique_templates) <= target_size:
            # If we have fewer templates than target size, use stratified sampling
            samples_per_template = target_size // len(unique_templates)
            remaining_samples = target_size % len(unique_templates)
            
            sampled_data = []
            for i, template in enumerate(unique_templates):
                template_data = labelled_logs[labelled_logs['EventTemplate'] == template]
                
                # Take a few samples from each template
                n_samples = samples_per_template + (1 if i < remaining_samples else 0)
                n_samples = min(n_samples, len(template_data))
                
                if len(template_data) > 0:
                    template_samples = template_data.sample(n=n_samples, random_state=42)
                    sampled_data.append(template_samples)
            
            labelled_logs = pd.concat(sampled_data, ignore_index=True)
        else:
            # If we have more templates than target size, just do random sampling
            labelled_logs = labelled_logs.sample(n=target_size, random_state=42)
